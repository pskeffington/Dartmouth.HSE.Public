...
Next, we use a list because its components can have different types. Each component remains a vector with one type. We retain jacks_list and give the components descriptive names so their roles are visible when we inspect them.
...
We convert the supplied values with as.integer(). This truncates toward zero rather than rounding: 8.88 becomes 8. We use my_integers because the exercise explicitly requests that output name; this corrects the working draft’s jacks_integers name.
...
We want values equal to 5 OR greater than 5555. Parentheses make each condition clear, while | combines them element by element. Values exactly equal to 5555 do not meet the second condition.
...
sum((number_vector == 5) | (number_vector > 5555))  # 13
```

## Exercise 9: Select three vector entries

We preserve the working draft’s jacks_vector name and select positions 1, 3, and 5. R uses one-based indexing. Saving the result gives us a reusable subset while leaving the original five-element vector intact.

```{r group-work-09}
# Lecture chunk references: 5, 11.
jacks_vector <- c("tacos", "burritos", "enchalada", "chalupa", "arroz")
jacks_subset <- jacks_vector[c(1, 3, 5)]
jacks_subset
```
...
We append five values because the matrix has five gene rows. Backticks allow the object name to contain a space. cbind() combines by position, so the new values must already follow the existing gene order.
...
Now we create a separate table with one participant per row. We keep jack.df and the working draft’s column labels, including spaces. check.names = FALSE preserves those labels; backticks are then needed when accessing a spaced name with $. The IDs are character identifiers, not quantities to calculate with.

```{r group-work-19}
# Lecture chunk references: 27.
jack.df <- data.frame(
  `Participant ID` = c("106", "107", "109"),
  Age = c(45, 23, 99),
  `Disease Status` = c("Case", "Control", "Case"),
  check.names = FALSE
)
jack.df
```
...
The dollar operator returns the named column as a vector. That is the appropriate structure for a later numerical summary. It differs from jack.df["Age"], which retains a one-column data frame.

```{r group-work-20}
# Lecture chunk references: 28.
jack.df$Age  # 45 23 99
```

## Exercise 21: Combine a function, loop, and conditional

Finally, we wrap the repeated operation in my_first_function(). At each row we inspect Disease Status: a case prints its Participant ID, and a control prints its Age. seq_len(nrow(df)) is a small refinement of the lecture’s index loop because it also handles zero rows. The explicit else-if avoids treating every non-case label as a control. This teaching example assumes complete Case/Control labels; imported data should be checked first.
...
my_first_function <- function(df) {
  ...
  for (i in seq_len(nrow(df))) {
    if (df$`Disease Status`[i] == "Case") {
      print(df$`Participant ID`[i])
    } else if (df$`Disease Status`[i] == "Control") {
      print(df$Age[i])
    }
  }
}
