...
Now we create a separate table with one participant per row. The working draft called it jack.df; participant.df describes its role and follows the lecture's count.df data-frame pattern. Its three columns map to Participant ID, Age, and Disease Status, but use lowercase snake_case so column access does not require backticks. The IDs are character identifiers, not quantities to calculate with.
